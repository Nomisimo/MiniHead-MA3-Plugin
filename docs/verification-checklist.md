...
- `Cmd(...)` — executes a command-line command (used for `NetworkSettings` → `Cmd("Menu 'RemoteIpEditor'")`, confirmed from grandMA3's own shipped user manual as the shortcut to the "My Network Interfaces" pop-up).
...
## Not yet live-tested: the `Window` command (experimental)

`Window` builds a full custom overlay window (title bar, header actions, a scrollable table with one row per head, global actions, footer) using `GetFocusDisplay().ScreenOverlay:Append('ClassName')` + dot-notation properties + `.PluginComponent`/`.Clicked`/`signalTable` for click handling. This is a materially different, less-certain API surface than everything else in this plugin:

- **Confirmed real**, not guessed: the class names used (`BaseInput`, `TitleBar`, `TitleButton`, `CloseButton`, `DialogFrame`, `ScrollBox`, `ScrollBarV`, `UILayoutGrid`, `Button`, `LineEdit`) appear both in two independent working community plugins ([BakaCowpoke/GrandMA3-Lua](https://github.com/BakaCowpoke/GrandMA3-Lua)) and in grandMA3's own shipped `message_box.uixml` (same TitleBar/DialogFrame/ScrollBox/UILayoutGrid structure MessageBox itself is built from).
- **Confirmed NOT a docked/ScreenContent window.** Read grandMA3's own `add_window.lua`: dockable window types come from a fixed, engine-built `ShowData().WindowTypes` list with no Lua API to register a new entry. Every `BaseInput`-rooted `.uixml` in the installation is in an overlay/input/dialog folder, never a docked one. So `Window` is an overlay that stays open only because its Lua task keeps running a wait-loop (`AutoClose='No'`) - not architecturally different from `Menu`'s dialogs in that sense, just a single persistent one instead of a sequence of modal ones.
- **Logic-tested, not render-tested.** The whole build runs cleanly against a from-scratch Lua mock of `Append`/property assignment/`.PluginComponent`+`.Clicked` (see the repo's test harness), and every registered click handler was verified to call the right underlying (already hardware-confirmed) function with the right arguments. What the mock *cannot* verify: whether grandMA3 actually renders this correctly, whether the row layout (8 elements per head, positioned by explicit X/Y) looks right at real screen scale, and whether the busy-wait loop that keeps the window open (`repeat ... until continue`) has any effect on the rest of the console while running.

**How to verify:** `Window` (or `Plugin <n> "Window"`). First check: does a window appear at all, or `Printf`s a `"Window build failed: ..."` message? If it appears, check the console stays responsive to other input while it's open, that rows show correct data, and that Apply/Identify/global buttons actually do something. Report back exactly what's on screen (a screenshot is most useful) - this is the biggest remaining unknown in the whole plugin.

**Known v1 limitation (by design, not a bug):** row and global actions call straight into the existing logic but don't refresh the window's own display - the table shows a snapshot from when it opened. Close and reopen to see updated state.
